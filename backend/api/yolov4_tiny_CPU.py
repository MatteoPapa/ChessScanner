...
model = tf.keras.models.load_model('fen_chess_data/models/model_best.h5', compile=False)

# Recompile it with a valid loss
model.compile(optimizer='adam', loss=CategoricalCrossentropy(reduction='sum'))
...
os.makedirs('fen_chess_data/data', exist_ok=True)
os.makedirs('fen_chess_data/models', exist_ok=True)
os.makedirs('fen_chess_data/predictions', exist_ok=True)
...
while True:
    ...
    if i and img.shape[0] > 0 and img.shape[1] > 0:
        ...
        for p in pts:
            p[0] += x_orig
            p[1] += y_orig
            cv2.circle(frame_orig, (int(p[0]), int(p[1])), 3, (0, 0, 255), -1)


        for i, j in [(0, 1), (0, 2), (1, 3), (2, 3)]:
            pt1 = (int(pts[i][0]), int(pts[i][1]))
            pt2 = (int(pts[j][0]), int(pts[j][1]))
            frame_orig = cv2.line(frame_orig, pt1, pt2, (0, 0, 255), 1)
